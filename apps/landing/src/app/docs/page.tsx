        ...
        <div className="space-y-16">
          ...
          <section id="installation" className="scroll-mt-24">
            ...
            <p className="text-text-secondary text-sm leading-relaxed mb-8">
              Requires <strong className="text-text-primary">React 18+</strong>, <strong className="text-text-primary">React Native 0.72+</strong>, and <strong className="text-text-primary">New Architecture</strong> enabled. Works with both bare React Native and Expo.
            </p>
            ...
            <div className="bg-bg-surface border border-border-default rounded-xl p-4">
              <p className="text-sm text-text-secondary">
                <strong className="text-text-primary">Note:</strong> Wrapping in <code className="font-mono text-xs bg-bg-elevated px-1.5 py-0.5 rounded">__DEV__</code> ensures Radar is fully tree-shaken from production builds. Call <code className="font-mono text-xs bg-bg-elevated px-1.5 py-0.5 rounded">init()</code> before any other code to capture all console logs and network requests.
              </p>
            </div>
          </section>
          ...
          <section id="configuration" className="scroll-mt-24">
            ...
            <p className="text-text-secondary leading-relaxed mb-4">
              To inspect Redux, Zustand, or other stores, pass them to <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">init()</code> via the <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">stores</code> option:
            </p>
            <CodeBlock
            ...
            <p className="text-text-secondary leading-relaxed mb-4">
              AsyncStorage is detected automatically. For MMKV, pass your instances to <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">init()</code>:
            </p>
            <CodeBlock
            ...
            <h2 className="font-semibold text-lg mb-3">Startup Metrics and TTI</h2>
            <p className="text-text-secondary leading-relaxed mb-4">
              Radar automatically measures JS bundle eval time. To track Time To Interactive (TTI), call <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">markInteractive()</code> when your app&apos;s main UI is ready:
            </p>
            <CodeBlock
            ...
            <div className="bg-bg-surface border border-border-default rounded-xl p-4 mb-8">
              <p className="text-sm text-text-secondary">
                <strong className="text-text-primary">Tip:</strong> If <code className="font-mono text-xs bg-bg-elevated px-1.5 py-0.5 rounded">markInteractive()</code> is not called within 10 seconds, startup metrics are sent without TTI.
              </p>
            </div>

            <h2 className="font-semibold text-lg mb-3">Custom Host and Port</h2>
            <p className="text-text-secondary leading-relaxed mb-4">
              By default, Radar connects to <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">localhost:8347</code> on iOS and <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">10.0.2.2:8347</code> on Android emulators. You can override these:
            </p>
            <CodeBlock
              code={`if (__DEV__) {\n  const { init } = require('radar-devtools');\n  init({\n    host: '192.168.1.100',\n    port: 8347,\n  });\n}`}
              className="mb-8"
            ...
            <p className="text-text-secondary text-sm leading-relaxed mb-4">
              All features work automatically. Radar uses <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">10.0.2.2</code> to reach your host machine.
            </p>

            <h3 className="font-medium text-text-primary mt-4 mb-2">Physical Device</h3>
            <p className="text-text-secondary text-sm leading-relaxed mb-4">
              Physical devices require your device and development machine to be on the same Wi-Fi network. You must specify your machine&apos;s IP address via the <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">host</code> option.
            </p>
            <div className="bg-bg-surface border border-border-default rounded-xl p-4 mb-4">
              <p className="text-sm text-text-secondary mb-3">
                <strong className="text-text-primary">Feature availability on physical devices:</strong>
              </p>
              <div className="space-y-3 text-sm">
                <div className="flex items-start gap-2">
                  <span className="text-status-success shrink-0 mt-0.5">&#10003;</span>
                  <p className="text-text-secondary">Console, Network, Component Tree, Profiler, State Management, Storage</p>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-status-warning shrink-0 mt-0.5">&#9888;</span>
                  <p className="text-text-secondary">Performance metrics (UI FPS, native RAM, CPU) and startup native launch time require the <code className="font-mono text-xs bg-bg-elevated px-1.5 py-0.5 rounded">RadarPerformance</code> TurboModule. JS heap and JS FPS work with Hermes.</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section id="features-overview" className="scroll-mt-24">
            ...
            <div className="space-y-8">
              ...
              <div className="border border-border-default rounded-xl p-6 bg-bg-secondary">
                ...
                <p className="text-text-secondary text-sm leading-relaxed">
                  Built-in MCP server exposes 18 tools to AI coding assistants. Let Claude, Cursor, Windsurf, or any MCP-compatible client read your app&apos;s runtime data and act on it. See the <a href="#mcp" className="text-accent hover:underline">AI Integration</a> section for setup.
                </p>
              </div>
            </div>
          </section>
          ...
          <section id="mcp" className="scroll-mt-24">
            ...
            <p className="text-text-secondary leading-relaxed mb-6">
              Radar includes a built-in <a href="https://modelcontextprotocol.io" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">Model Context Protocol</a> (MCP) server that exposes all devtools data as tools for AI coding assistants. When Radar is running, your AI can read console logs, inspect network requests, browse component trees, analyze profiler data, and more.
            </p>

            <h2 className="font-semibold text-lg mb-3">Setup</h2>
            <p className="text-text-secondary leading-relaxed mb-6">
              The MCP server starts automatically when you open Radar and listens on port <code className="font-mono text-xs bg-bg-surface px-1.5 py-0.5 rounded">8348</code>. Pick your AI tool below and run the CLI command or add the config manually:
            </p>

            <McpProviderSwitcher />

            <h2 className="font-semibold text-lg mt-10 mb-3">What Can You Ask?</h2>
            <p className="text-text-secondary leading-relaxed mb-4">
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                <div key={example} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-bg-surface border border-border-subtle">
                  <span className="text-accent font-mono text-sm">&gt;</span>
                  <span className="text-text-secondary text-sm font-mono">{example}</span>
                </div>
              ))}
            </div>
            ...
            <div className="bg-bg-surface border border-border-default rounded-xl p-4">
              <p className="text-sm text-text-secondary">
                <strong className="text-text-primary">Note:</strong> The MCP server requires Radar to be running and connected to at least one device. All tools accept an optional <code className="font-mono text-xs bg-bg-elevated px-1.5 py-0.5 rounded">deviceId</code> parameter — if omitted, they return data from all connected devices.
              </p>
            </div>
          </section>
        </div>
